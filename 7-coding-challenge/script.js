...
(function () {
  ...
  questionsArray.push(new Question('Is JavaScript the coolest programming language in the world?',
    ['Yes', 'No'], 0));
  questionsArray.push(new Question('What is the name of this course\'s teacher?',
    ['John', 'Micheal', 'Jonas'], 2));
  questionsArray.push(new Question('What does best describe coding?',
    ['Boring', 'Hard', 'Fun', 'Tediuos'], 2));
  const selectedQuestion = Math.floor(Math.random() * questionsArray.length);
  ...
  displayQuestion(questionsArray[selectedQuestion]);
  const answer = parseInt(prompt('Please select the correct answer.'));
  checkAnswer(questionsArray[selectedQuestion], answer);
})();
*/
/*
--- Expert level ---
...
(function () {
  ...
  function nextQuestion () {
    ...
    if (answer !== 'exit') {
      questionsArray[n].checkAnswer(parseInt(answer), keepScore);
      nextQuestion();
    }
  }
  nextQuestion();
